...
const CoursesCommunity = dynamic(() => import("./CoursesCommunity"), {
  loading: () => <CustomLoader />,
});

const BookCallSection = dynamic(() => import("./BookCall"), {
  loading: () => <CustomLoader />,
});
...
function FooterColumn({
  ...
  return (
    <>
      <p className="mb-3 lg:mb-5 text-[16px] font-semibold leading-none tracking-[-0.03em] text-white sm:text-[17px] md:text-[18px] lg:text-[22px]">
        {title}
      </p>

      <ul className="space-y-0.5 sm:space-y-1.5 xl:space-y-2">
        {links.map((item) => (
          <li key={item.label}>
            <Link
              href={item.href}
  ...
  return (
    <>
      ...
      <footer className="overflow-hidden bg-black text-white">
        <div className="section-container mx-auto w-full pb-8 pt-7 sm:pb-10 sm:pt-8 md:pb-12 md:pt-10 lg:pt-12">
          <div className="grid grid-cols-1 gap-y-10 sm:grid-cols-2 sm:gap-x-8 sm:gap-y-12 xl:grid-cols-[1.5fr_2.8fr_1fr] xl:gap-x-14">
            ...
            <div className="order-1 text-center sm:text-left">
              ...
              <p className="mx-auto mt-3 max-w-[420px] text-[14px] font-medium leading-[1.45] tracking-[-0.025em] text-white/90 sm:mx-0 sm:text-[15px] md:text-[16px]">
                Supreme Coach helps coaches and firms launch courses, build
                communities, and sell from one simple platform.
              </p>
            </div>
              ...
              {securityItems.map((item, i) => (
                ...
                >
                  <Image
                    ...
                    width={item.width}
                    height={item.height}
                    className={`h-7 w-7 object-contain sm:h-8 sm:w-8 md:h-9 md:w-9 lg:h-10 lg:w-10 ${
                      i !== 4 ? "opacity-[0.6]" : ""
                    }`}
                  />
                </div>
